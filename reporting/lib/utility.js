...
class Utility {
	...
	get customRequest () {
		...
		return request.defaults({
			headers: { 'User-Agent': 'Discover Automated Tests' }
		});
	}
	...
	*/
	...
	// async testUrlStatusSync (urlString) {
	// 	if ( !new String(urlString).includes('https') ) {
	// 		urlString = `${browser.config.baseUrl}${urlString}`;
	// 	}

	// 	return await this.customRequest.get(urlString, (error, response, resp) => {
	// 		return {
	// 			href: urlString,
	// 			status: response.statusCode || 0,
	// 			message: response.statusMessage || 'Request Error',
	// 		};
	// 	});
	// }
